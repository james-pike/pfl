...
export default component$(() => {
  ...
  useOnDocument('click', $((event) => {
    // Check if the event.target is not null
    const target = event.target as HTMLElement | null;
    if (store.isNavOpen && target && !target.closest('.sidebar')) {
      store.isNavOpen = false; // Close the sidebar
    }
  }));
  ...
  return (
    ...
    >
            ...
            >
              ...
              >
                <path
                  ...
                  stroke-width="2"
                  d={store.isNavOpen ? 'M6 18L18 6M6 6l12 12' : 'M4 6h16M4 12h16m-7 6h7'}
                ></path>
              </svg>
            </button>
          </div>
      ...
      {/* Sidebar */}
      <div class={`sidebar ${store.isNavOpen ? 'open' : ''}`}>
